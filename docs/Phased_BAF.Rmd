...
![](BAF_phasing.svg){width="717" height="384"}
...
![HG00405_metrics](Phased_BAF_files/figure-gfm/unnamed-chunk-1-1.png)
...
snp_stats <- data.table::fread("UPD_snp_summary.tsv")
nchr <- length(unique(snp_stats$chrom))
colored_cat <- unique(snp_stats$B_InhFrom)
names(colored_cat) <- c("black", "#E69F00","#39918C","red","#999999")
F2_snps <- snp_stats%>%
  group_by(B_InhFrom)%>%
  summarise(aver_freq=round(mean(freq),2),
            sd_ratio=sd(freq),
...
![UPD_case](Phased_BAF_files/figure-gfm/unnamed-chunk-2-1.png)

![](UPD_case.png){width="724"}

...
ReadGVCF <- function(path_to_gVCF,ref_genome=ref_genome,param = param,target_spacing){
  ...
  GT.anno <- GT %>% 
    ...
    #                          TRUE ~ "#999999"))%>%
    # maternal and paternal specific allele  
    mutate(B_col = case_when(B_Index %in% c(6,7,9,10) ~ "#E69F00",
                             B_Index %in% c(1,2,4,5) ~ "#39918C",
                             B_Index %in% c(19:20) ~ "red",
                             B_Index %in% c(12:18) ~ "black",
                             TRUE ~ "#999999"))%>%
    ...
    #                            TRUE ~ "Notphased"))%>%
    mutate(B_InhFrom=case_when(B_Index %in% c(6,7,9,10) ~ P1_ID,
                               B_Index %in% c(1,2,4,5) ~ P2_ID,
                               B_Index %in% c(19:20) ~ "ME",
                               B_Index %in% c(12:18) ~ "AOH_signal",
                               TRUE ~ "Notphased"))
  ...
  AD.anno <- AD%>%
    ...
    mutate(index_REF_RD=sapply(index,"[[",1),
           ...
           p1_ALT_Freq=p1_ALT_RD/(p1_REF_RD+p1_ALT_RD),
           p2_ALT_Freq=p2_ALT_RD/(p2_REF_RD+p2_ALT_RD))%>%
  ...
  AD.anno <- AD.anno[,c("index_ALT_RD","pr_count","p1_ALT_RD","p1_count","p2_ALT_RD","p2_count","pr_ALT_Freq","p1_ALT_Freq","p2_ALT_Freq","pr_absBAF","likelyDN")]
  merged <- cbind(GT.anno ,AD.anno)%>%
    mutate(P1_phased_BAF=case_when(B_Index %in% c(6,7,9,10) ~ pr_ALT_Freq),
           P2_phased_BAF=case_when(B_Index %in% c(1,2,4,5) ~ pr_ALT_Freq))
